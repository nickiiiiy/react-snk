...
function App() {
  ...
  const onAddToCart= async (obj)=>{
    try{
      ...
      if(findItem){
        ...
        await axios.delete(`https://656b1e0edac3630cf727b554.mockapi.io/cart/${findItem.id}`);
      } else{
        const {data}=await axios.post('https://656b1e0edac3630cf727b554.mockapi.io/cart',obj);
  ...
  const onAddFavorite =async(obj)=>{
    try{
      // const findFavorite=favorite.find((favObj)=>Number(favObj.id)==Number(obj.id))
      if(favorite.find((favObj)=>Number(favObj.id)==Number(obj.id))){
        setFavorite(prev => prev.filter((item)=>Number(item.id)!==Number(obj.id) ));
        await axios.delete(`https://656ef5c86529ec1c62370ac3.mockapi.io/favorites/${obj.id}`);
      }
      else{
        const {data} = await axios.post('https://656ef5c86529ec1c62370ac3.mockapi.io/favorites',obj);
  ...
  return (
    <AppContext.Provider value={{cartItems,favorite,items,isFavoriteAdded,isItemAdded,onAddFavorite,onAddToCart, setCartOpened,setCartItems}}>
